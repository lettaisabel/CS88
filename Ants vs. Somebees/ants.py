...
# END Problem 12
    ...
    def construct(cls, gamestate):
        ...
        """
        # BEGIN Problem 12
        if gamestate.created is None:
            ant = super(QueenAnt,cls).construct(gamestate)
            if ant is not None:
                ant.is_created = True
                gamestate.created = ant
            return ant
        return None
        # END Problem 12
...
class Bee(Insect):
    ...
    def move_to(self, place):
        ...
        self.scarelen = max(self.scarelen-1, 0)
    ...
    def action(self, gamestate):
        ...
        gamestate -- The GameState, used to access game state information.
        """
        destination = self.place.entrance if self.scarelen else self.place.exit
        ...
        if self.blocked():
            ...
            if self.slowed == 0 or gamestate.time&1 == 0:
                self.move_to(destination)
        self.slowed = max(self.slowed-1,0)
